...
const b=new BrainCPU(g,{seed:37}),runs=[];
for(let k=0;k<8;k++){
 const rates=Float32Array.from([180,k<4?0:30,0,0,0]);
 const r=b.batch(100,rates,k===5);
...
brain=TorchBrain(5,np.array([0,0,1,2,4,5]),np.array([0,1,1,2,3]),np.array([80,70,15,80,25]),np.array([1,1,-1,1,0]),seed=37,device=args.device)
...
for k,expected in enumerate(reference['runs']):
    actual=brain.batch(100,np.array([180,0 if k<4 else 30,0,0,0],dtype=np.float32),k==5)
...
print(json.dumps({'status':'PASS','device':args.device,'steps':800,'seed':37,'maxVoltageErrorMv':max_error,'resetToRest':True}))
