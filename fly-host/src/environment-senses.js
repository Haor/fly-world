...
export function lightSamples(pose, {light = 1, lightAngle = 0}, rock, occlusion = 0) {
  ...
  const visibility = (pose.y||0) < .4 && along > 0 && along < distance && cross < rock.radius ? .15 : 1;
  const local = Math.max(0, light) * visibility / (1 + (Math.hypot(distance,pose.y||0) / 20) ** 2) * (1 - clamp(occlusion, 0, 1));
