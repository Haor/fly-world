...
export class FlyScene {
  constructor(canvas) {
    ...
    this.trail = new THREE.Line(
      pg,
      new THREE.LineBasicMaterial({ color: 0x7ee2d5, transparent: true, opacity: 0.22 }),
    );
  ...
  setShadows(enabled) {
    ...
    // Shadow sampling is compiled into materials; invalidate it when toggling.
  ...
  render(now) {
    if (this.disposed) return;
    this.frame = requestAnimationFrame(this.render);
    if (document.hidden || this.visible === false || !this.gait) return;
