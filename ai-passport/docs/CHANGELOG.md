...
- Add bilateral odor input and an independent navigation aid to the host station; retain separate left/right rates in the cloud contract. Offline firmware behavior is unchanged.
- Separated browser display shadows from the short neural looming stimulus.

- Added the dedicated Fly World observation station and explicit transfer of USB control between browser pages.

- Added continuous habitat feedback, configurable exploration drive, contact-gated neural feeding, looming escape, and absolute USB world synchronization.

- Added Fly World: a pixel habitat with a USB MaleCNS companion, offline feeding/play/rest, bounded serial messages, and local persistence.
