...
- 电脑观测站增加双侧嗅觉输入与独立寻食辅助，云端协议保留左右输入；离线固件行为不变。
- 将浏览器阴影显示与短时神经逼近刺激分开。

- 增加专属 Fly World 观测站，以及浏览器页面间的 USB 控制权主动切换。

- 增加持续环境反馈、可调探索驱动、接触条件下的神经进食、逼近逃逸及 USB 世界绝对坐标同步。

- 新增 Fly World：像素栖息地、USB MaleCNS 伴侣、离线喂食/玩耍/休息、限长串口协议与本地状态保存。
